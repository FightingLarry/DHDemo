...
/**
 * 由于ServiceManager里面的sCache里面存储的 IBinder类型基本上都是BinderProxy 因此,
 * ServiceManager的使用者调用getService之后不会直接使用这个map 而是先将他使用asInterface转成需要的接口
 * <p/>
 * asInterface函数的代码告诉我们, 它会先使用这个BinderPRoxy查询本进程是否有Binder对象 如果有就使用本地的, 这里恰好就是一个hook点
 * <p/>
 * 我们让所有的查询都返回一个"本地Binder"对象
 * <p/>
 * 当然,这是一个假象, 我们给它返回的Binder对象自然是符合要求的(要么是本地Binder,要么是Binder代理) 只不过,我们对需要hook的API做了处理
 * <p/>
 ...
 * @date 16/2/15
 */
public class AMSBinderProxyHookHandler implements InvocationHandler {

    private static final String TAG = "AMSBinderProxyHookHandler";
    ...
    public AMSBinderProxyHookHandler(IBinder base) {
        this.base = base;
        try {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        ...
        if ("queryLocalInterface".equals(method.getName())) {
            ...
            return Proxy.newProxyInstance(proxy.getClass().getClassLoader(),
                    ...
                    // 因此, 正常情况下
                    // 在asInterface里面会由于BinderProxy的queryLocalInterface返回null导致系统创建一个匿名的代理对象,
                    // 这样我们就无法控制了
                    ...
                    new Class[] {this.iinterface}, new AMSBinderHookHandler(base, stub));
        }
